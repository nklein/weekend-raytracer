...
(defun csg-union-image-5d (samples-per-pixel)
  "This example renders an image cube looking at the union
of three spheres."

  (let* ((aspect-ratios '(1 16/9 64 64))
         (width 320)
         (center (vec 13 10 5 0 0))
         (lookat (vec 2 1/2 1/2 1/2 0))
         (orientation (list (vec 0 1 0 0 0)))
         (viewport '(32/9 2 3/2 3/2))
         (permutation '(0 2 1 3))
    ...
    (let* ((world (list (sphere (vec 0 0 -1000 0 0) 998
                                (lambertian (color 1/2 1/2 3/4)))
                        (sphere (vec -3 -2 0 7/8 1/8) 2
                                (metal (color 3/4 1/4 1/4) 0.0))
                        (sphere (vec -5 2 0 1 -1/8) 2
                                (metal (color 1/4 3/4 1/4) 0.0))
                        (csg-union
                         (sphere (vec 2 1 0 0 1/8) 2
                                 (dialectric (color 1 1 1) 3/2))
                         (sphere (vec 2 0 1 0 -1/8) 2
                                 (lambertian (color 4/10 2/10 1/10)))
                         (sphere (vec 2 0 0 1 0) 2
                                 (metal (color 9/10 8/10 3/10) 0.0)))))
           (img (render camera world :samples-per-pixel samples-per-pixel)))
    (write-image #P"CSG-Union-5d" img
                 :border-width 2
                 :border-color (vector 0 0 0)
                 :permutation permutation
                 :cutoff 2
